 ...
 */
export const main = `
  <div id="refresh" onclick="%%" title="Aktualisiere Tabellendaten">↻</div>
  <div id="data"></div>
`;

/**
 * returns the HTML template for the data table
 * @param {Object} questions - question-focused data
 * @returns {TemplateResult} HTML template for the question-focused table
 */
export function table( questions ) {
  return html`
    <table class="table table-hover mb-0">
      <tbody>
        ${ repeat( Object.values( questions ).filter( question => question.total ), question => question.key, question => questionTemplate( question ) ) }
      </tbody>
    </table>
    <button id="points-chart" class="btn btn-info btn-block">&empty; Zeige Punktedurchschnitt</button>
  `;
}
    ...
    <tr class="bg-light question" data-key="${ question.key }">
      <th title="MC Frage">
        <div class="d-flex align-items-center">
          ...
          <div class="ml-2 text-wrap">${ question.title }</div>
        </div>
      </th>
      <td>
        <span title="Zeige Diagramm">
      ...
      <td>
        <span title="Richtig beantwortet">
      ...
      <td>
        <span title="Falsch beantwortet">
          ...
          <span>${ question.total - question.correct }</span>
        </span>
      </td>
      <td>
        <span title="Durchschnittlich erreichte Punkte">
          <b class="text-primary mr-1 average">&empty;</b>
          <span>${ Math.round( question.points.average * 10 ) / 10 }</span>
        </span>
      </td>
    </tr>
    ${ repeat( Object.values( question.answers ), answer => answer, answerTemplate ) }
  `;
}
    ...
    <tr class="answer" data-key="${ answer.key }">
      <td title="MC Antwort">
        <div class="d-flex align-items-center">
          ...
          <div class="ml-2 text-wrap">${ answer.text }</div>
        </div>
      </td>
      <td>
        <span title="Lösung">
      ...
      <td>
        <span title="Falsch angekreuzt">
          <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-x text-danger" viewBox="0 0 16 16">
            <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
          </svg>
          <span>${ answer.total - answer.correct }</span>
        </span>
      </td>
      <td></td>
    </tr>
